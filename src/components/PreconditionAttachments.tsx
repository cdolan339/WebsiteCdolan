 ...
 *
 * An attachment upload/display section shown under Preconditions.
 * Matches the Attachments component's UI (upload dialog, download, lightbox, etc.)
 *
 * Two modes:
...
import { Paperclip, Upload, Trash2, X, Eye, Download, FileText, File, ClipboardPaste } from 'lucide-react'
...
export function PreconditionAttachments({ testCaseId, readOnly = false, pendingImages, onPendingChange }: Props) {
  ...
  // Upload via dialog (existing test case)
  ...
  // Add to pending list (new test case — dialog mode)
  const handlePendingUpload = async (file: File, title: string) => {
    if (!onPendingChange) return
    const displayName = title + (file.name.match(/\.[^.]+$/)?.[0] || '')
    const pending: PendingImage = {
      file,
      preview: URL.createObjectURL(file),
      name: displayName,
    }
    onPendingChange([...(pendingImages || []), pending])
    setDialogOpen(false)
  }
  ...
  const savedImages = attachments.filter((a) => isImage(a.mimetype))
  const savedDocs = attachments.filter((a) => !isImage(a.mimetype))
    ...
    <div style={{ marginTop: '14px' }}>
      ...
      {savedImages.length > 0 && (
        <div style={{
          ...
          gap: '8px',
          marginBottom: savedDocs.length > 0 ? '10px' : '8px',
        }}>
          {savedImages.map((att) => (
            <div
              key={att.id}
      ...
      {dialogOpen && (
        <UploadDialog
          onUpload={isPending ? handlePendingUpload : handleUpload}
